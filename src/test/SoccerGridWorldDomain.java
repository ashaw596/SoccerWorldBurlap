 ...
 *
 */
public class SoccerGridWorldDomain implements DomainGenerator {
	 ...
	 * Constant for the name of the north action
	 */
	public static final String							ACTIONPASSNORTH = "passnorth";
	
	/**
	 * Constant for the name of the south action
	 */
	public static final String							ACTIONPASSSOUTH = "passsouth";
	
	/**
	 * Constant for the name of the east action
	 */
	public static final String							ACTIONPASSEAST = "passeast";
	
	/**
	 * Constant for the name of the west action
	 */
	public static final String							ACTIONPASSWEST = "passwest";
	
	
	
	/**
	 * Constant for the name of the pass action
	 */
	public static final String							ACTIONPASS = "pass";
	
	/**
	 * Constant for the name of the shoot action
	 */
	public static final String							ACTIONSHOOT = "shoot";
	 ...
	 */
	public SoccerGridWorldDomain(int width, int height){
		this.width = width;
		this.height = height;
	 ...
	 */
	public SoccerGridWorldDomain(int [][] map){
		this.setMap(map);
		this.setDeterministicTransitionDynamics();
	}
	...
	public Domain generateDomain() {
		...
		new MovePassAction(ACTIONPASSNORTH, domain, this.transitionDynamics[0]);
		new MovePassAction(ACTIONPASSSOUTH, domain, this.transitionDynamics[1]);
		new MovePassAction(ACTIONPASSEAST, domain, this.transitionDynamics[2]);
		new MovePassAction(ACTIONPASSWEST, domain, this.transitionDynamics[3]);
		
		new PassAction(ACTIONPASS, domain);
		
		new ShootAction(ACTIONSHOOT, domain);
	...
	public static State getOneAgentNoLocationState(Domain d, int ax, int ay){
		...
		s.addObject(new ObjectInstance(d.getObjectClass(CLASSAGENT), CLASSAGENT+0));
		SoccerGridWorldDomain.setAgent(s, ax, ay);
	...
	protected void move(State s, int xd, int yd){
		...
		ObjectInstance passAgent = s.getObjectsOfTrueClass(CLASSLOCATION).get(0);
		int px = passAgent.getDiscValForAttribute(ATTX);
		int py = passAgent.getDiscValForAttribute(ATTY);
	 ...
	 * @param yd the attempted new Y position of the agent
	 */
	protected void movePass(State s, int xd, int yd){
		
		ObjectInstance passAgent = s.getObjectsOfTrueClass(CLASSLOCATION).get(0);
		int ax = passAgent.getDiscValForAttribute(ATTX);
		int ay = passAgent.getDiscValForAttribute(ATTY);
		
		int nx = ax+xd;
		int ny = ay+yd;
		
		ObjectInstance agent = s.getObjectsOfTrueClass(CLASSAGENT).get(0);
		int px = agent.getDiscValForAttribute(ATTX);
		int py = agent.getDiscValForAttribute(ATTY);
		...
		passAgent.setValue(ATTX, nx);
		passAgent.setValue(ATTY, ny);
	}
	...
	public class ShootAction extends Action{
		 ...
		 */
		public ShootAction(String name, Domain domain){
			super(name, domain, "");
		...
		protected State performActionHelper(State st, String[] params) {
			...
			double fail = Math.pow(distance(ax, ay, width, height/2+.5), 2)/(Math.pow(6, 2)) + 0.2;
			

			ObjectInstance passAgent = st.getObjectsOfTrueClass(CLASSLOCATION).get(0);
			int px = passAgent.getDiscValForAttribute(ATTX);
			int py = passAgent.getDiscValForAttribute(ATTY);
			if (roll>fail) {
				agent.setValue(ATTX, 1);
				agent.setValue(ATTY, 1);
				passAgent.setValue(ATTX, 1);
				passAgent.setValue(ATTY, 1);
			} else {
				agent.setValue(ATTX, 0);
				agent.setValue(ATTY, 0);
				passAgent.setValue(ATTX, 0);
				passAgent.setValue(ATTY, 0);
			}
			return st;
		}
		...
		public List<TransitionProbability> getTransitions(State st, String [] params){
			State ns = st.copy();
			ObjectInstance passAgent = ns.getObjectsOfTrueClass(CLASSLOCATION).get(0);
			ObjectInstance agent = ns.getObjectsOfTrueClass(CLASSAGENT).get(0);
			int px = passAgent.getDiscValForAttribute(ATTX);
			int py = passAgent.getDiscValForAttribute(ATTY);
			int ax = agent.getDiscValForAttribute(ATTX);
			int ay = agent.getDiscValForAttribute(ATTY);
			double fail = Math.min(Math.pow(distance(ax, ay, width, height/2+.5), 2)/(Math.pow(6, 2)) + 0.2, 1);
			...
			State end = st.copy();
			ObjectInstance endPassAgent = end.getObjectsOfTrueClass(CLASSLOCATION).get(0);
			ObjectInstance endAgent = end.getObjectsOfTrueClass(CLASSAGENT).get(0);
			endAgent.setValue(ATTX, 0);
			endAgent.setValue(ATTY, 0);
			endPassAgent.setValue(ATTX, 0);
			endPassAgent.setValue(ATTY, 0);
			
			transitions.add(new TransitionProbability(ns, 1-fail));
			transitions.add(new TransitionProbability(end, fail));
			return transitions;
		}
	}
	...
	public class PassAction extends Action{
		 ...
		 * @param directions the probability for each direction (index 0,1,2,3 corresponds to north,south,east,west, respectively).
		 */
		public PassAction(String name, Domain domain){
			super(name, domain, "");
			this.rand = RandomFactory.getMapped(0);
		}
		...
		protected State performActionHelper(State st, String[] params) {
			...
			double roll = rand.nextDouble();
			double fail = Math.pow(distance(ax, ay, px, py), 3)/(Math.pow(8, 3));
			if (roll>fail) {
				agent.setValue(ATTX, px);
				agent.setValue(ATTY, py);
				passAgent.setValue(ATTX, ax);
				passAgent.setValue(ATTY, ay);
			} else {
				agent.setValue(ATTX, 0);
				agent.setValue(ATTY, 0);
				passAgent.setValue(ATTX, 0);
		...
		public List<TransitionProbability> getTransitions(State st, String [] params){
			...
			int ax = agent.getDiscValForAttribute(ATTX);
			int ay = agent.getDiscValForAttribute(ATTY);
			double fail = Math.min(Math.pow(distance(ax, ay, px, py), 3)/(Math.pow(8, 3)),1);
			List <TransitionProbability> transitions = new ArrayList<TransitionProbability>();
	 ...
	 *
	 */
	public class MovePassAction extends Action{
		 ...
		 * @param directions the probability for each direction (index 0,1,2,3 corresponds to north,south,east,west, respectively).
		 */
		public MovePassAction(String name, Domain domain, double [] directions){
			super(name, domain, "");
		...
		protected State performActionHelper(State st, String[] params) {
			...
			int [] dcomps = SoccerGridWorldDomain.this.movementDirectionFromIndex(dir);
			SoccerGridWorldDomain.this.movePass(st, dcomps[0], dcomps[1]);
		...
		public List<TransitionProbability> getTransitions(State st, String [] params){
			...
			for(int i = 0; i < directionProbs.length; i++){
				...
				State ns = st.copy();
				int [] dcomps = SoccerGridWorldDomain.this.movementDirectionFromIndex(i);
				SoccerGridWorldDomain.this.movePass(ns, dcomps[0], dcomps[1]);
	...
	public class MovementAction extends Action{
		...
		protected State performActionHelper(State st, String[] params) {
			...
			int [] dcomps = SoccerGridWorldDomain.this.movementDirectionFromIndex(dir);
			SoccerGridWorldDomain.this.move(st, dcomps[0], dcomps[1]);
		...
		public List<TransitionProbability> getTransitions(State st, String [] params){
			...
			for(int i = 0; i < directionProbs.length; i++){
				...
				State ns = st.copy();
				int [] dcomps = SoccerGridWorldDomain.this.movementDirectionFromIndex(i);
				SoccerGridWorldDomain.this.move(ns, dcomps[0], dcomps[1]);
	...
	public class WallToPF extends PropositionalFunction{
		...
		public WallToPF(String name, Domain domain, String[] parameterClasses, int direction) {
			super(name, domain, parameterClasses);
			int [] dcomps = SoccerGridWorldDomain.this.movementDirectionFromIndex(direction);
			xdelta = dcomps[0];
			ydelta = dcomps[1];
		}
		...
		public boolean isTrue(State st, String[] params) {
			...
			if(cx < 0 || cx >= SoccerGridWorldDomain.this.width || cy < 0 || cy >= SoccerGridWorldDomain.this.height || SoccerGridWorldDomain.this.map[cx][cy] == 1 || 
					(xdelta > 0 && (SoccerGridWorldDomain.this.map[ax][ay] == 3 || SoccerGridWorldDomain.this.map[ax][ay] == 4)) || (xdelta < 0 && (SoccerGridWorldDomain.this.map[cx][cy] == 3 || SoccerGridWorldDomain.this.map[cx][cy] == 4)) ||
					(ydelta > 0 && (SoccerGridWorldDomain.this.map[ax][ay] == 2 || SoccerGridWorldDomain.this.map[ax][ay] == 4)) || (ydelta < 0 && (SoccerGridWorldDomain.this.map[cx][cy] == 2 || SoccerGridWorldDomain.this.map[cx][cy] == 4)) ){
				return true;
			}
	...
	public static void main(String[] args) {
		...
		SoccerGridWorldDomain gwdg = new SoccerGridWorldDomain(11, 11);
